/**
 * Script to manage the page form.
 */

require(['jquery', 'witycms/admin'], function($) {
	/**
	 * Stackoverflow
	...
	function formatURL(idLang) {
		var value = $(namespace + '.lang_' + idLang + ' .title').val();

		if (value === '') {
			$(namespace + '.lang_' + idLang + ' .meta_title').val('');
			$(namespace + '.lang_' + idLang + ' .url').val('');
			return;
		}

		$(namespace + '.lang_' + idLang + ' .meta_title').val(value);
		...
		if (value !== '') {
			var parent = $(namespace + '#parent option:selected').attr('data-page-url');
			if (parent) {
				parent += '/';
			} else {
				parent = '';
	...
	$(namespace + '.update-url').click(function() {
		var $this = $(this),
			$urlInput = $this.closest('.input-group').find('input'),
			idLang = $urlInput.data('lang');
